...
class Client {
    ...
    async create(value) {
        ...
        if (existingClient) {
            ...
            if (existingClient.client_phone === value.client_phone) {
                throw new Error(`The phone number '${value.client_phone}' is already in use.`);
            } 
        }
        ...
        return {
            status: true,
            message: "Client created & email has been sent to the client registered email address",
        };
    }
    ...
    async resizeImg(file) {
        ...
        image.resize(612, 383)  // Resize the image to 250px width
    ...
    async setup(data) {
        ...
        await shlSessions.upsert({
            acd_session: `${new Date().getFullYear() - 1}/${new Date().getFullYear()}`,
            service_id : data.service_id,
    ...
    async activateCLientService(service_id) {
        return await this.client_service.update({service_status: 1}, {where: {service_id: service_id}});
    }
