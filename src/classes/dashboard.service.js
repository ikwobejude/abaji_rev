...
class Admin {
    ...
    async officeExpectedGraph(service_id) {
        let sql = `
        SELECT 
            sum(r.grand_total) AS amount,
             MAX(toffice.tax_office) AS label 
        FROM revenue_upload AS r
        INNER JOIN tax_offices AS toffice ON toffice.tax_office_id = r.tax_office_id OR toffice.tax_office = r.tax_office_id
        WHERE r.service_id = ${service_id} AND rate_year = ${this.year}
        GROUP BY toffice.tax_office_id`;
    ...
    async officePaymentGraph(service_id) {
        let sql = `
           SELECT 
            sum(r.grand_total) AS amount,
             MAX(toffice.tax_office) AS label 
        FROM revenue_upload AS r
        INNER JOIN tax_offices AS toffice ON toffice.tax_office_id = r.tax_office_id OR toffice.tax_office = r.tax_office_id
        WHERE r.service_id = ${service_id} AND rate_year = ${this.year} AND r.payment_status = 1 GROUP BY toffice.tax_office_id`;
