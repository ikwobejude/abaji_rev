...
class DashboardController {
  ...
  static async revenueTaxOffice(req, res) {
    try {
      if(req.user.service_type === "State") {
        const data = await admin.officeExpectedGraph(req.user.service_id);
        res.status(200).json(data);
      } else {
        const data = await admin.streetExpectedGraph(req.user.service_id);
        res.status(200).json(data);
  ...
  static async paymentsStreet(req, res) {
    try {
      if(req.user.service_type === "State") {
        const data = await admin.officePaymentGraph(req.user.service_id);
        res.status(200).json(data);
      } else {
